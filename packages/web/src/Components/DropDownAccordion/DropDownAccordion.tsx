...
const AccordionLabel = ({ label, description, image }: AccordionLabelProps) => (
  <Group noWrap>
...
export const DropDownAccordion = () => {
  ...
  const items = charactersList.map(({ image, label, description, content }) => (
    <Accordion.Item
      label={
        <AccordionLabel image={image} label={label} description={description} />
      }
      key={label}
    >
      <Text>{content}</Text>
